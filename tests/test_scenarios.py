...
class TestCarServiceScenarios:
    ...
    def test_list_cars_by_available_status(self, tmpdir: str, card_data: list[Car], model_data: list[Model]):
        ...
        available_cars = [
            car for car in card_data if car.status == CarStatus.available]
        
        assert sorted(available_cars, key=lambda car: car.vin) == service.get_cars(CarStatus.available)
    ...
    def test_top_3_models_by_sales(self, tmpdir: str, card_data: list[Car], model_data: list[Model]):
        ...
        top_3_models = [
            ...
            ModelSaleStats(model_name="Pathfinder",
